...
async function sponsorRoach(roachId, amount) {
    ...
    if (allowance.lt(amount)) {
        const approveTx = await roachContract.approve(ROACH_RALLY_CONTRACT, amount, { gasLimit: 100_000 });
        await approveTx.wait();
    }

    const contract = new ethers.Contract(ROACH_RALLY_CONTRACT, roachRallyABI, signer);
    console.log("Race contract",ROACH_RALLY_CONTRACT)
    const ret = await contract.sponsorRoach(roachId, amount, address, true, { gasLimit: 350_000 });
    return ret;
}
